...
\label{ch::motivated_intro::defn::dual_transf}
    ...
    The \textit{dual of $\ff$}, also called the \textit{transpose of $\ff$}, is the linear function $\ff^*:W^* \rightarrow V^*$ defined by $\ff^*(\psi) = \psi \circ \ff$.
\end{defn}

The following theorem reveals why the dual is sometimes referred to as the transpose.

\begin{theorem}
    (The dual is represented by the transpose matrix).
