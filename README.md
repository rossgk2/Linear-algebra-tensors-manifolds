...
### Orientation
* We fully explain the concept of orientation by building it from the ground up. This elucidates the confusing "right hand rule" which is often thrust upon students in courses on multivariable calculus, linear algebra, and physics. While it's common for other linear algebra texts to briefly mention how orientation formalizes the right hand rule, they never present the theorem that really makes it click.
  * The theorem: when the counterclockwise angle from one vector to another exceeds $\pi$, the orientation- and thus the cross product- of the 2-tuple containing the vectors changes sign. Later, when we introduce the cross product, it quickly follows from this theorem that the sign of the cross product changes when the counterclockwise angle from one vector to another exceeds $\pi$. This- in my opinion- is much easier than remembering how to twist your right hand in the way specified by the right hand rule.
