...
\begin{itemize}
	...
	Then, add a remark about how one can more formally construct tensor and wedge product spaces by using $F(V \times W)/\sim$ for various $\sim$. See Warner for how to do this, and add a section on quotient vector spaces and $F(V \times W)/\sim$ to the appendix. 
...
\textbf{Big new idea}: add a ``Duality'' section after the ``Multilinear functions and tensor product spaces'' section and before the ``Introduction to $(p, q)$ tensors'' section. Now that I know about how to introduce dual spaces really intuitively from the perspective of duality, I think that this is a better introduction than just seeing dual spaces fall out of the decomposition of a linear function. That decomposition certainly explains why $\LLLL(V \rightarrow K)$ is important, but not why functionalizing it in a definition $V^* := \LLLL(V \rightarrow K)$ is intuitive. The intuitive duality introduction does. And this way, by the time we come across $\LLLL(V \rightarrow K)$ in the decomposition, we are already familiar with it. Here's the outline:
\begin{itemize}
	\item Follow the argument from ``Another perspective on dual spaces'', except for the part that makes use of natural isomorphisms from later in this chapter.
	\item In place of that part, use a dimensionality argument. We have $(V^*)^* := \LLLL(\LLLL(V \rightarrow W) \rightarrow W)$, and so for finite-dimensional spaces we have $\dim((V^*)^*) = \dim(\LLLL(V \rightarrow W))\dim(W) = \dim(V)\dim(W)^2$. Thus if we want $(V^*)^* \cong V$ we need $\dim(W) = 1$.
\end{itemize}
